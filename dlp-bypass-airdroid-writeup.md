...
**Author:** Alvaro Martinez | IT Infrastructure Specialist  
...
This writeup documents a **Data Loss Prevention (DLP) policy bypass** identified in a simulated corporate endpoint environment. A Windows 10 laptop configured with BitLocker Full Disk Encryption, BitLocker To Go on removable storage, Cisco AnyConnect Full Tunnel VPN enforcement, and restricted user privileges was found to be vulnerable to local file exfiltration via an unmanaged WiFi hotspot and the AirDroid Personal web interface (port 8888).

The scenario demonstrates that organizations which implement multiple independent security controls — each targeting a specific threat — can still be exposed when **local wireless traffic remains uncontrolled**. BitLocker To Go prevented USB exfiltration. Cisco AnyConnect blocked internet-based exfiltration. Neither control addressed the local WiFi channel.

**Result:** A confidential Word document was successfully transferred from the restricted endpoint to an external device without admin privileges, without USB access, and without corporate internet connectivity — bypassing all three active DLP controls.
...
| USB Policy | Removable storage allowed — but BitLocker To Go enforced (files copied to USB are automatically encrypted via Group Policy) |
| User Privileges | Standard user (no local admin rights) |
| Disk Encryption | BitLocker enabled (Full Disk Encryption via Group Policy) |
| Mobile Device | Android smartphone with AirDroid Personal (free) |

> **Note on BitLocker and BitLocker To Go:** Two encryption layers were active on the endpoint. **BitLocker** (Full Disk Encryption) protects data at rest — it secures the drive if the hardware is stolen or accessed offline. **BitLocker To Go** was enforced via Group Policy on removable storage: USB drives were allowed, but any file copied to them was automatically encrypted, making it unreadable on any external PC without the recovery key stored in Active Directory. Despite both controls being active, neither prevented exfiltration via the local WiFi channel documented in this writeup.
...
**3.1 BitLocker To Go — USB Encryption Confirmed**
A USB flash drive was connected to the laptop. The device mounted successfully and the file was copied without restriction. However, when the USB was connected to a different PC outside the corporate environment, the copied `.docx` file could not be opened. Windows displayed an access error. This confirmed an active **BitLocker To Go policy**: removable storage is permitted, but all files written to external drives are automatically encrypted. The file is only accessible on domain-joined machines holding the recovery key via Active Directory — rendering USB as a practical exfiltration channel ineffective.
...
### What was controlled:
- ✅ USB exfiltration → Mitigated via BitLocker To Go (files encrypted on removal)
- ✅ Internet access → Blocked via Cisco AnyConnect Full Tunnel
- ✅ Local administrator access → Restricted
- ✅ Disk data at rest → Protected via BitLocker Full Disk Encryption
...
### The gap:
The security architecture implemented **three independent controls** targeting different attack vectors: BitLocker To Go for USB exfiltration, Cisco AnyConnect Full Tunnel for internet-based exfiltration, and BitLocker FDE for physical theft. Each control addressed its intended threat. However, **none of them controlled local wireless traffic** between the endpoint and a personal mobile hotspot. A file transferred via AirDroid over a WiFi hotspot exits the endpoint in plaintext, through an unmonitored channel, with no policy enforcement in place.
...
| **Bypasses BitLocker To Go** | Files transferred via AirDroid exit the endpoint in plaintext — BitLocker To Go only encrypts files written to USB, not files transferred over WiFi |
| **Bypasses Full Disk Encryption** | BitLocker FDE protects against physical theft, not against an authenticated user transferring files over a local wireless channel |
| **Policy bypass** | All DLP controls (USB encryption, VPN, FDE) rendered ineffective against this vector |
...
*Alvaro Martinez | IT Infrastructure Specialist | Google Cybersecurity · Cisco CyberOps*  
